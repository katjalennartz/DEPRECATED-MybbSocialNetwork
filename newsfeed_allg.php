...
$haspage = $db->fetch_field($db->simple_select("users","userpage", "uid = '$thisuser'"), "userpage");
 ...
 $newsfeedlink_allg = " - <a href=\"newsfeed_allg.php\">allgemeiner newsfeed</a>";
...
// Variablen für Benachrichtigung
	...
	redirect("newsfeed_allg.php", "Newsfeed");
}      
...
$count_sql = $db->query("SELECT count(sp.social_id) cnt FROM (SELECT uid, social_profilbild, social_nutzername, username u_username FROM ".TABLE_PREFIX."users) u 
JOIN
(SELECT social_id,uid,social_date,userpageid,social_post FROM ".TABLE_PREFIX."socialpost up) sp 
ON u.uid=sp.uid ORDER BY social_id DESC");
$row = $count_sql->fetch_assoc();
$total_pages = ceil($row['cnt'] / $per_page);
for ($i=1; $i<=$total_pages; $i++) {  // print links for all pages

$multipage .= "<a href='newsfeed_allg.php?page=".$i."'>".$i."</a> - ";
}; 

//Query um die neusten Posts zu sehen
//$sql = "SELECT * FROM ".$datatable." ORDER BY ID ASC LIMIT $start_from, ".$results_per_page;

$social_postquery=$db->query("SELECT * FROM (SELECT uid, social_profilbild, social_nutzername, username u_username FROM ".TABLE_PREFIX."users) u 
JOIN
(SELECT social_id,uid,social_date,userpageid,social_post FROM ".TABLE_PREFIX."socialpost up) sp 
ON u.uid=sp.uid ORDER BY social_id DESC LIMIT ".$start_from.", ".$per_page."");
